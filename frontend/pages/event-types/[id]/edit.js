...
export default function EditEventType() {
  ...
  const [form, setForm] = useState({ title:'', description:'', duration:30, slug:'', color:'#0ea5e9', is_active:true });

  useEffect(() => {
    if (!id) return;
    getEventType(id).then(data => { setForm(data); setLoading(false); })
      .catch(() => { toast.error('Not found'); router.push('/dashboard'); });
  }, [id]);
  ...
  if (loading) return <Layout title="Edit"><div className="flex items-center justify-center h-64"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-gray-900" /></div></Layout>;

  return (
    <Layout title="Edit Event Type">
